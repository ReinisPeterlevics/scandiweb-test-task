...
class Furniture extends Product
{
  ...
  private function isHeightValid($height, $errors)
  {
    ...
    if (strlen(substr(strrchr($height, "."), 1)) > 0 || $height < 0 || !is_numeric($height)) {
  ...
  private function isWidthValid($width, $errors)
  {
    ...
    if (strlen(substr(strrchr($width, "."), 1)) > 0 || $width < 0 || !is_numeric($width)) {
  ...
  private function isLengthValid($length, $errors)
  {
    ...
    if (strlen(substr(strrchr($length, "."), 1)) > 0 || $length < 0 || !is_numeric($length)) {
